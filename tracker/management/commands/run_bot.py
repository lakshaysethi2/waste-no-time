...
class Command(BaseCommand):
    ...
    async def log_now(self, chat_id, tag, notes, update_or_query):
        ...
        # Requirement: ack must be silent (no sound) and include quick-settings keyboard
        keyboard = await self.get_keyboard(chat_id)
    ...
    async def get_keyboard(self, chat_id, page=0, per_page=ACTIVITY_KEYBOARD_PAGE_SIZE):
        ...
        # Quick-settings rows: required by spec — 15s, 30s, 1m, 2m, 5m, 10m, on, off
        # These mirror /settings but are available on every main keyboard so users can adjust
        # check interval and pause/resume without opening settings.
        mt = await asyncio.to_thread(self.get_kv, chat_id, "mt") or "on"
        keyboard.append(
            [
                InlineKeyboardButton("15s", callback_data="key:ci:15"),
                InlineKeyboardButton("30s", callback_data="key:ci:30"),
                InlineKeyboardButton("1m", callback_data="key:ci:60"),
            ]
        )
        keyboard.append(
            [
                InlineKeyboardButton("2m", callback_data="key:ci:120"),
                InlineKeyboardButton("5m", callback_data="key:ci:300"),
                InlineKeyboardButton("10m", callback_data="key:ci:600"),
            ]
        )
        # Explicit on/off as requested, plus a friendly toggle label
        keyboard.append(
            [
                InlineKeyboardButton("on", callback_data="key:mt:on"),
                InlineKeyboardButton("off", callback_data="key:mt:off"),
                InlineKeyboardButton(
                    "⏸ Pause" if mt == "on" else "▶ Resume",
                    callback_data="toggle:mt",
                ),
            ]
        )
